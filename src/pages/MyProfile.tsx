...
function Toggle({
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      >
        <span
          className={`absolute top-[3px] w-[14px] h-[14px] rounded-full bg-paper border border-ink/20 transition-transform ${
      ...
      <div className="cursor-pointer" onClick={() => onChange(!checked)}>
        ...
        {description && <p className="font-archivo text-xs text-ink/40 mt-0.5 leading-snug">{description}</p>}
      </div>
    </div>
  )
}
